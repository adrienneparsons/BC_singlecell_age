...
2. The ASPEN framework is described in plain language in "ASPEN Pseudocode.txt". If you would like a more generalized script of the ASPEN framework, we provide "ASPEN Generalized.R," which is a more basic script designed to be easily editable to apply with allternative single cell RNA sequencing data.
...
Once the gene expression files are formatted or obtained from FigShare, “1 Bulk RNA DE Genes and GSEA” performs differential gene expression analysis between the ER+ and TNBC donors <45 and >65 years of age with stage I-III tumors. Once tables of the necessary donors’ gene expression are made, the genes with the highest variance across the entire old and young donor pool per subtype are identified in METABRIC and low abundance genes are filtered out in TCGA, and differential gene expression analysis is performed using limma. The results of this analysis are visualized as a volcano plot:

![Volcano Plots.](/www/VolcanoPlots.png)

The highly genes are then ranked by fold change difference (highest positive fold change to lowest negative fold change, irrespective of p-value) and GSEA is performed using the .gmt files found on MSigDB or in the Data folder of this repository. The results, which use the fgsea package to find Normalized Enrichment Scores, are visualized as bubble plots:
...
To better understand the activity of specific cell types within young and aged breast tumor microenvironments, these scripts utilized the package CellChat (Jin et al. Nature Communications, 2021). First, CellChat objects are generated from the single-cell atlas in the file “0.2 Create CellChat Objects.R”. The objects created include all ER+ donors, all TNBC donors, then all ER+ donors ≤55, all ER+ >55, all TNBC donors ≤55, and all TNBC donors >55. Because these samples are unsorted, we set the argument computeCommunProb() to be TRUE. This script takes >60 minutes to run, so for ease of replication, we have included the resulting CellChat objects used in downstream analyses in the Data folder of this repository.
