...
## MAJOR CELL TYPES (Fig. 4A and Fig. 4B)
# Load the cellchat objects previously generated
# The RDATA files can be found in Dropbox > Single-cell_breast_cancer > Github > BC_singlecell_age > Data
...
## MINOR CELL TYPES (Fig. 4C and Fig. 4D)
# Load the cellchat objects previously generated
# The RDATA files can be downloaded from Dropbox > > Single-cell_breast_cancer > Github > BC_singlecell_age > Data
# Create an object list
# DO A SEPARATE ANALYSIS FOR TNBC AND ER
...
# Remove the following code comment for ER (ER young has not B cells Naive, so we need to lift up)
